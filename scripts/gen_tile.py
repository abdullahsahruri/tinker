...
def emit_tile_tlg_hc(weights, out_path: Path) -> dict:
    ...
    parts = [
        ...
        f"seed={WEIGHT_SEED}, neurons={N_NEURONS}.",
...
def emit_tile_tlg_ld(weights, out_path: Path) -> dict:
...
def emit_tile_handopt_hc(weights, out_path: Path) -> dict:
...
def emit_tile_handopt_ld(weights, out_path: Path) -> dict:
...
def write_manifest(out_path: Path, variant_logic: str, variant_mode: str,
                   weights, thresholds, info: dict) -> None:
