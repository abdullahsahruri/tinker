...
complete RISC-V SoC under software control.
...
($6 \to 3$) that Yosys synthesizes into a network of
...
comparator. Fig.~\ref{fig:tlg-decomp} visualizes the decomposition.
...
per-quadrant median binarization, four parallel branches each map a
...
given up to binarization itself.
...
simulation time zero. A vectorized Python integer model serves as
...
Yosys synthesizes the design against the
...
OpenSTA blackbox stub for the behavioral macro views, and removal of
...
summarized in Table~\ref{tab:verification}. The vectorized Python
...
boundary near zero and is the expected behavior of the integer
quantization, not a bug. The post-P\&R SoC RTL simulation then matches
...
\texttt{max\_ff\_n40C\_1v95} corner. The sixteen-image inference
testbench of the 7\,\(\times\)\,7 deployment (Section~\ref{sec:eval:energy})
is replayed against the post-P\&R netlist, which the 14\,\(\times\)\,14
deployment leaves byte-identical; the chip-pin VCD
is annotated through OpenSTA's \texttt{read\_vcd}, supplemented with
activities for the 47 of 1\,210 interior VCD signals that survive
synthesis as named nets, and
\texttt{report\_power} is decomposed into per-instance buckets by
a bucketization pass. The remaining interior nets use propagated
activity, so combinational power is likely under-counted by up to
about 5\,\%. Total post-P\&R power is
...
15.10\,mW (32.0\,\%). The TLG-mapped tile, including its Wishbone
wrapper, is the largest active
...
The remaining 1.98\,mW (4.2\,\%) is not attributed to any bucket and
appears as ``Other'' in Fig.~\ref{fig:power_breakdown}.
\begin{figure}[t]
  \centering
...
image: the post-P\&R netlist, sign-off artifacts, and clock are
...
lower default-activity power. Neither standalone tile meets the
100\,MHz target at the slow corner (73.81 and 77.51\,MHz); the
100\,MHz operating point holds at the nominal TT corner, as for the
SoC (Section~\ref{sec:impl:signoff}). Because this ablation is tile-level
...
\input{tables/table_tlg_ablation}
...
hidden by storage overhead at SoC scale, and tile-level optimization
...
The sign-off result is also deliberately bounded: the present
